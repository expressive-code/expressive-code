import { CodeFenceMetadata, LineMarker, InlineMarker, MarkerType, LineRange } from './types';
...
export class MetadataParser {
   ...
   */
  parse(metaString: string): CodeFenceMetadata {
    const result: CodeFenceMetadata = {
      language: '',
      lineMarkers: [],
      inlineMarkers: [],
      useDiffSyntax: false,
   ...
   */
  private parseLineMarkers(metaString: string): LineMarker[] {
    ...
    while ((match = lineMarkerPattern.exec(metaString)) !== null) {
      const type = (match[1] as MarkerType) || 'mark';
   ...
   */
  private parseLineRanges(rangeString: string): LineRange[] {
   ...
   */
  private parseInlineMarkers(metaString: string): InlineMarker[] {

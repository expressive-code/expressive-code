import { CodeFenceMetadata, MarkerMatch, MarkerType } from './types';

/**
 * Matches markers in code block content and returns positions for semantic tokens
 */
export class MarkerMatcher {
   ...
   */
  findMatches(content: string, metadata: CodeFenceMetadata, startLine: number): MarkerMatch[] {
  ...
  private findInlineMatches(
    ...
    if (isRegex) {
      ...
      // Plain text search
      let index = 0;
      while ((index = line.indexOf(pattern as string, index)) !== -1) {
        matches.push({
          start: index,
          end: index + (pattern as string).length,
        });
        index += (pattern as string).length;
      }
    }
   ...
   */
  private findDiffMarkers(lines: string[], startLine: number): MarkerMatch[] {
    const matches: MarkerMatch[] = [];

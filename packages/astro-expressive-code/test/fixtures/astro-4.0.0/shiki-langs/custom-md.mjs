...
export default {
	...
	repository: {
		...
		fenced_code_block: {
			patterns: [
				{
					include: '#fenced_code_block_test_language',
				},
				{
		...
		fenced_code_block_test_language: {
			begin: '(^|\\G)(\\s*)(`{3,}|~{3,})\\s*(?i:(test-language)((\\s+|:|,|\\{|\\?)[^`]*)?$)',
			beginCaptures: {
				3: {
					name: 'punctuation.definition.markdown',
				},
			...
			patterns: [
				{
					begin: '(^|\\G)(\\s*)(.*)',
					contentName: 'meta.embedded.block.test-language',
					patterns: [
						{
							include: 'source.test-language',
						},
					],
					while: '(^|\\G)(?!\\s*([`~]{3,})\\s*$)',
				},

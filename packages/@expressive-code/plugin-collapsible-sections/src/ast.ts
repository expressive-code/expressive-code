import { formatTemplate, setInlineStyle, ExpressiveCodeBlock } from '@expressive-code/core'
...
export function sectionizeAst({
	...
	;[...sections]
		...
		.forEach(({ from, to }) => {
			...
			if (minIndent > 0 && minIndent < Infinity) setInlineStyle(details, '--ecIndent', `${minIndent}ch`)

...
import { PluginGutterElement, getRenderEmptyLineFn, renderLineToAst, splitLineAtAnnotationBoundaries } from '../src/internal/render-line'
...
describe('renderLineToAst()', () => {
	...
	describe('Simple inline annotations', () => {
		test('Line starting with a class annotation', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code"><0>Wow</0>, I am rendered!</span></span>`)
		})
		...
		test('Line ending with a class annotation', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, I am <0>rendered!</0></span></span>`)
		})
	...
	describe('Line-level annotations', () => {
		test('Single line-level class annotation', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass} del"><span class="code">Wow, I am rendered!</span></span>`)
		})
		...
		test('Multiple line-level class annotations', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass} del mark"><span class="code">Wow, I am rendered!</span></span>`)
		})
	...
	describe('Combined line-level and inline annotations', () => {
		test('With class annotations', () => {
			...
			line.addAnnotation(new ClassNameAnnotation({ addClass: 'mark' }))
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass} del mark"><span class="code">Wow, <2>I am <1>rendered</1>!</2></span></span>`)
		})
	...
	describe('Multiple non-intersecting annotations', () => {
		test('Line starting and ending with an annotation', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code"><0>Wow</0>, I am <1>rendered!</1></span></span>`)
		})
	})
	...
	describe('Intersecting annotations', () => {
		test('Two annotations with matching boundaries', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, I am <1><0>rendered</0></1>!</span></span>`)
		})
		test('Two annotations where the second is fully contained in the first', () => {
			const line = new ExpressiveCodeLine(testText)
			annotateMatchingTextParts({ line, partsToAnnotate: ['I am rendered', 'am'] })
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, <0>I </0><1><0>am</0></1><0> rendered</0>!</span></span>`)
		})
		test('Two annotations where the first is fully contained in the second', () => {
			const line = new ExpressiveCodeLine(testText)
			annotateMatchingTextParts({ line, partsToAnnotate: ['am', 'I am rendered'] })
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, <1>I <0>am</0> rendered</1>!</span></span>`)
		})
		test('Two partially intersecting annotations', () => {
			const line = new ExpressiveCodeLine(testText)
			annotateMatchingTextParts({ line, partsToAnnotate: ['Wow, I am', 'I am rendered!'] })
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code"><0>Wow, </0><1><0>I am</0> rendered!</1></span></span>`)
		})
		test('Three annotations where part indices must move after merging', () => {
			const line = new ExpressiveCodeLine(testText)
			annotateMatchingTextParts({ line, partsToAnnotate: ['am', 'I am rendered', '!'] })
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, <1>I <0>am</0> rendered</1><2>!</2></span></span>`)
		})
	})
	...
	describe('Ensures that empty lines are visible', () => {
		test('Empty lines are rendered with a line break inside', () => {
			...
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">\n</span></span>`)
		})
		test('Empty lines also work with line-level annotations', () => {
			const line = new ExpressiveCodeLine('')
			line.addAnnotation(new ClassNameAnnotation({ addClass: 'del' }))
			expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass} del"><span class="code">\n</span></span>`)
		})
	})

	describe('Gutter elements', () => {
		test('Wraps rendered gutter elements in a gutter container before the code', () => {
			const line = new ExpressiveCodeLine(testText)
			expect(renderLineToHtml(line, [testGutterElement])).toEqual(
				`<span class="${codeLineClass}"><span class="gutter"><span class="ln">1</span></span><span class="code">Wow, I am rendered!</span></span>`
			)
		})
	})
	...
	describe('Respects render phases', () => {
		describe('With class annotations', () => {
			test('Annotation #0 with phase "latest" is rendered after #1 with "normal"', () => {
				...
				expect(renderLineToHtml(line)).toEqual(`<span class="${codeLineClass}"><span class="code">Wow, <0>I <1>am</1> rendered</0>!</span></span>`)
			})
...
describe('getRenderEmptyLineFn()', () => {
	test('Renders an empty line without gutter elements', () => {
		const { lineAst, gutterWrapper, codeWrapper } = getRenderEmptyLineFnWrapper([])()
		expect(toHtml(lineAst)).toEqual(`<span class="${codeLineClass}"><span class="code"></span></span>`)
		expect(gutterWrapper).toBeUndefined()
		expect(codeWrapper).toBe(lineAst.children[0])
	})
	test('Renders an empty line with gutter element placeholders', () => {
		const { lineAst, gutterWrapper, codeWrapper } = getRenderEmptyLineFnWrapper([testGutterElement])()
		expect(toHtml(lineAst)).toEqual(`<span class="${codeLineClass}"><span class="gutter"><span class="ln"></span></span><span class="code"></span></span>`)
		expect(gutterWrapper).toBe(lineAst.children[0])
		expect(codeWrapper).toBe(lineAst.children[1])
	})
})
...
const testGutterElement: PluginGutterElement = {
	pluginName: 'TestPlugin',
	gutterElement: {
		renderLine: ({ lineIndex }) => h('span.ln', `${lineIndex + 1}`),
		renderPlaceholder: () => h('span.ln'),
	},
}

function renderLineToHtml(line: ExpressiveCodeLine, gutterElements: PluginGutterElement[] = []) {
	return toHtml(renderLineToAstWrapper(line, gutterElements))
}

function renderLineToAstWrapper(line: ExpressiveCodeLine, gutterElements: PluginGutterElement[] = []) {
...
function getRenderEmptyLineFnWrapper(gutterElements: PluginGutterElement[]) {
	return getRenderEmptyLineFn({
		gutterElements,
		...baseTestContext,
	})
}

const baseTestContext = {

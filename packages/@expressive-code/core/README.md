...
- [API](#api)
	...
	- [`ExpressiveCodeThemeOptions`](#expressivecodethemeoptions)
...
- **Note to website authors:** The default class `expressive-code` is also added to all wrapper elements, providing you with a way to target all code blocks in CSS regardless of the config options.
...
- **Note:** You usually don't need to create code blocks manually. Instead, you can pass `ExpressiveCodeBlockOptions` to the [`render`](#async-renderinput-renderinput-options-renderoptions) method of the `ExpressiveCodeEngine` class, and the engine will create the code blocks for you.
...
Represents a single code line that is part of an `ExpressiveCodeBlock`. Provides access to the line's code plaintext and any line-level and inline annotations.

#### `new ExpressiveCodeLine(text: string)`

- Creates a new instance of a code line.
- **Important:** You should never need to create code lines manually outside of unit tests. Instead, you most likely want to pass code as a string to the [`render`](#async-renderinput-renderinput-options-renderoptions) method of the `ExpressiveCodeEngine` class, and the engine will create code blocks and lines for you.
- Arguments:
	- `text: string`
...
- Inserting text at a column works by setting `columnStart` and `columnEnd` to the same column, and setting `newText` to the text to be inserted.
- Deleting the selected range of characters works by setting `newText` to an empty string.
- May throw an error if not allowed in the parent block's current [`state`](#state-expressivecodeprocessingstate).
...
- Note: To save on bundle size, this constructor does not support loading themes bundled with Shiki by name (e.g. `dracula`). Instead, you need to import Shiki's `loadTheme` function yourself, use it to load its bundled theme (e.g. `themes/dracula.json`), and pass the result to this constructor.
- Higher-level packages like `remark-expressive-code` automatically handle this for you, so you only need to worry about it if you are using this core package directly.

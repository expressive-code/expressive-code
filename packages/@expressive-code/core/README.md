...
All config options that can be passed to the [constructor](#new-expressivecodeengineconfig-expressivecodeengineconfig) are also available on the instance as read-only properties.
...
- ##### `async getBaseStyles()`
  ...
  Please note that the styles contain references to CSS variables, which must also be added to the page. These can be obtained by calling [`getThemeStyles`](#async-getthemestyles).
...
- ##### `async getThemeStyles()`
  ...
  Please note that these styles must be added to the page together with the base styles returned by [`getBaseStyles`](#async-getbasestyles).
...
- ##### `type: VSCodeThemeType`
  ...
  > **Note**:
  > Shiki's special `"css"` theme type is not supported by Expressive Code. If you want to use CSS variables for certain styling aspects, have a look at the `styleOverrides` property instead, which allows you to override style colors with CSS variables used on your site.
...
- ##### `styleOverrides: StyleOverrides`
  ...
  For more information, see the `styleOverrides` config option of the [`ExpressiveCodeEngine` constructor](#expressivecodeengine-constructor).
...
A concrete implementation of [`ExpressiveCodeAnnotation`](#expressivecodeannotation) that allows you to apply theme-dependent inline styles to code.
